...
- `presentCall(args)` → a `ToolCallView` (the PENDING card):
  ...
  - `{ card: 'diff', title, diffs, locations? }` — your call creates or modifies a file. `diffs: [{ path, oldText, newText, oldStart?, newStart? }]` (`oldText: null` for a new file; optional starts are 1-based hunk line numbers) renders as an inline diff card. (tool-fs `write`/`edit`.)
- `presentResult(args, { content, isError, meta? })` returns the completed card:
  ...
  - `diff` supplies applied hunks, often derived by `output.presentationMeta` and carried in persisted `result.meta` so replay reproduces them. A hunk may include optional 1-based `oldStart`/`newStart` line numbers for a numbered UI gutter. Mutation tools keep a diff result because the completed view replaces the pending card.
