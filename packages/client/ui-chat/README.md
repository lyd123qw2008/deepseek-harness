...
Chat supplies file and HTTP(S) navigation through one `MarkdownDelegateProvider` around its node list. Assistant Markdown file links settle into the configured Chat file target—Host default application by default, or Sidebar preview when selected in Settings → General—including references to unmodified files. Relative paths resolve in the viewed Session's workspace; absolute paths retain the same Session's filesystem access. A `#L24` or `#L24-L30` suffix remains part of the authored link; the Sidebar target previews the resolved file, while the Host target hands the resolved path to the Host opener. Missing files are reported by the selected target.
