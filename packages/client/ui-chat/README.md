...
Use this package to render Session conversations in a browser, including historical images, localized actions, and restored scroll position. Compact display folds completed-turn process rows while keeping final answers and context visible; packed historical Assistant runs remain collapsed. Local transcript and steering submissions appear immediately and disappear when authoritative Session records arrive; queued submissions stay outside Chat. Markdown file links and produced-file mentions resolve against the Session workspace and use the Host's default application. File mentions receive the viewed Session ID and closing-turn owner, so inherited-history links can address the fork. The package does not modify model requests.
...
Chat supplies file and HTTP(S) navigation through one `MarkdownDelegateProvider` around its node list. Assistant Markdown file links and HTTP(S) links use the configured Chat opening target from Settings → General: Host default application is the default, while Sidebar preview selects the in-app target. Relative file paths resolve in the viewed Session's workspace; absolute paths retain the same Session's filesystem access. A `#L24`, `#L24-L30`, or `:24` suffix remains part of the authored link; the Sidebar target previews local files at the requested line and opens web links in a right-Sidebar Browser tab, while the Host target hands local files to the Host opener and web links to the system browser. If the Sidebar Browser is unavailable, web links fall back to the system browser. Missing files are reported by the selected target.

Modified clicks on HTTP(S) links retain the native external-link behavior. Sent file references and skills confirmed by the message’s logged invocation also open in the right Sidebar. File paths use the viewed Session; skill names resolve through its current input-trigger source. Both use the prose file-link dotted underline on hover or focus. Sessions, directories, and command labels remain non-navigating references.
