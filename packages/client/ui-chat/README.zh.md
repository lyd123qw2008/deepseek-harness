...
使用本包可在浏览器中渲染已记录的 Session 对话，包括历史图片、本地化操作和滚动位置恢复。紧凑显示会收起已完成轮次的过程行，同时保持最终答案和有用上下文可见；已打包的历史 Assistant 连续消息保持收起。本地 transcript（文本记录）与 steering（中途引导）提交会立即显示在原区域，并在权威会话记录到达时消失；排队中的提交始终不进入 Chat。Markdown 文件链接和产出文件提及会相对于 Session 工作区解析，并调用 Host 的系统默认应用打开。文件提及提供方接收当前查看的会话 ID 与收尾轮次属主，因此继承历史中的链接可以指向 fork。本包不修改模型请求。
