...
Mount this plugin alongside `ui-conversation`; a finished turn then ends with the produced-files row between the closing message's body and its action footer. Each chip opens the file through the owner's `openFile`, which the chat view resolves against the Session workspace and sends to the Host's default desktop application. The row offers no folder action; an omitted-file remainder is a label only.
...
- [Workspace file links](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md) — the decision behind the produced-files row and its Host open path; [the restoration note](../../../.agents/notes/implemented/bug-fix/2026-09-09-restore-host-file-opening.md) records the current default.
...
- **Directories have no destination** — chips list files only, so an omitted-file remainder is a label rather than a folder action.
