...
`dsh-session-format-v0-to-v1` decodes the released-v0 JSONL record language one physical row at a time and converts it into the shared-layout v1 format. The edge preserves validated header and event facts except `version: 0` becoming `version: 1`, applies the finite legacy normalizers that v0 persistence accepted, and promotes released subagent descriptor version 2 payloads to version 3. The package freezes the v0 reader, the strict v1 migration target validator, and a vocabulary-neutral v1 physical codec a later edge can reuse; `payload-validation.ts` and `relationships.ts` pin every first-party event's payload members and lifecycle pairings.
