...
- **Native lock support is platform-specific** — `fs-ext` is an optional dependency loaded only by the POSIX lease path; Windows uses the `koffi`-backed semaphore path. POSIX deployment and first write require a working `fs-ext` addon.
