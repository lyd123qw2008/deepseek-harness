...
export class FsSandboxController {
   ...
   * requires no approval. Known equal-or-narrower stale escalation fields run
   * under the standing policy; every authority-changing request still
   * validates the escalation argument pairing first. The calling session's
   * cwd is always carried as the workspace root.
