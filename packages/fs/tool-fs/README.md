...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification`. A known target already covered by the standing policy is normalized and runs under it even when an inherited `justification` is blank or absent; a mode-changing retry validates its paired non-empty justification and resolves through `ctx.approval`. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
