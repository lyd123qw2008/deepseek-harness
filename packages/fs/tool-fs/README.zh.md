...
当 `ctx.fs.sandboxMode` 表明提供方施加沙箱限制时，write/edit 会公开 `sandbox_permissions` 与 `justification`。常驻策略已覆盖的已知目标会被规范化处理并按该策略运行，即使继承的 `justification` 为空或缺失；会改变模式的重试会校验成对且非空的理由，并通过 `ctx.approval` 处理。策略归属方会贡献与具体能力无关的常驻策略；工具结果仍保留针对具体操作的拒绝与重试引导。
