...
describe('pi-ai SDK retry boundary', () => {
  ...
  it('filters malformed response facts instead of emitting invalid retry metadata', async () => {
    ...
    let responseIndex = 0
    streamSimple.mockImplementation((model: Model<Api>, _context: PiContext, options: SimpleStreamOptions) => {
  ...
  it('preserves a terminal status while filling missing response retry facts', async () => {
    let streamIndex = 0
    streamSimple.mockImplementation((model: Model<Api>, _context: PiContext, options: SimpleStreamOptions) => {
