...
The adapter forces pi-ai's SDK `maxRetries` to zero so one `stream()` call makes one model-generation request. Codex routes are pinned to `transport: 'sse'`, even when a profile configures any other transport; this prevents pi-ai's WebSocket `response.create` fallback from replaying a request before the first event. The composed `dsh-llm-retry` extension owns visible retries and their durable `llm/retry` records, so an adapter failure produces one generation attempt before the agent-level policy decides whether to retry. The removed profile fields `maxRetries` and `maxRetryDelayMs` fail load instead of silently multiplying or hiding the separately composed agent-level retry budget. Idle expiry aborts the SDK's stable request signal and surfaces `TIMEOUT`; an earlier caller abort remains `ABORTED`.
...
- pi-ai reports failures as in-stream error events; these map to `finish {kind:'error'|'aborted', failure}` chunks. For patched OpenAI Responses and Codex, the bundled pi-ai patch preserves provider error codes and HTTP status; non-2xx response headers supply Retry-After and request-id when the API exposes them. A recognized code wins over status. When no code is present, explicit quota, billing, or model-disabled wording remains terminal before status fallback; otherwise status wins over legacy message text. APIs without either metadata field still use text and usage signals to distinguish terminal `QUOTA`, transient `RATE_LIMIT`, and `CONTEXT_WINDOW_EXCEEDED`. A terminal `stop` whose message carries no content blocks maps to a `finish {kind:'error'}` with code `EMPTY_RESPONSE` (retried by default policy) instead of a successful empty message.
- `server_error`, a transient structured overload code, or an HTTP 5xx response maps to transient `SERVER`; `rate_limit_exceeded` or 429 maps to `RATE_LIMIT`; `stream_transform_error` and narrow HTTP/2 reset markers map to `TRANSPORT`. Normal retry policy includes these codes unless the provider's `retryableCodes` removes them. The exact terminal codes `server_is_overloaded`, `model_disabled`, and `model_unavailable` remain non-retryable; `overloaded`, `overloaded_error`, `model_overloaded`, `model_busy`, and `slow_down` remain transient `SERVER` signals. Invalid-request, quota, and unrecognized metadata without a status remain non-retryable; generic overload text is not a retry signal.
...
- **Provider metadata coverage follows pi-ai** — the local patch preserves provider code and HTTP status for OpenAI Responses and Codex, and the adapter captures Retry-After and request-id from exposed non-2xx response headers. Other protocols may still flatten failures to text, and mid-stream errors may have no HTTP response facts.
