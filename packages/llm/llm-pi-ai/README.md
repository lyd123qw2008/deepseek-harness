...
- pi-ai reports failures as in-stream error events; these map to `finish {kind:'error'|'aborted', failure}` chunks. For OpenAI Responses, the bundled pi-ai patch preserves the provider `error.code` and HTTP status: a recognized code wins over status, and status wins over legacy message text. APIs without either metadata field still use text and usage signals to distinguish terminal `QUOTA`, transient `RATE_LIMIT`, and `CONTEXT_WINDOW_EXCEEDED`. A terminal `stop` whose message carries no content blocks maps to a `finish {kind:'error'}` with code `EMPTY_RESPONSE` (retried by default policy) instead of a successful empty message.
- `server_error` or an HTTP 5xx response maps to transient `SERVER`; `rate_limit_exceeded` or 429 maps to `RATE_LIMIT`. Normal retry policy includes both codes unless the provider's `retryableCodes` removes them. Unrecognized metadata and legacy text remain `PI_AI_ERROR`.
