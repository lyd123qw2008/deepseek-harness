...
| `sandbox_permissions` | string enum | Advertised only when a sandboxing executor is mounted (`ctx.shell.sandboxMode` defined). The wider sandbox mode for a one-shot retry of a command the sandbox just denied. A known target already covered by the effective per-session mode is ignored and runs under the standing policy even when an inherited `justification` is blank or absent; only a mode-changing request needs the narrowest sufficient wider mode, a non-empty `justification`, and user approval through `ctx.approval` BEFORE execution. |
| `justification` | string | Required with a mode-changing `sandbox_permissions` request: one sentence for the user explaining why this exact command needs the wider access. A known target already covered by the standing policy is normalized without an approval prompt. |
