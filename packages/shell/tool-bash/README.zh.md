...
| `sandbox_permissions` | string enum | 仅当已挂载的执行器启用沙箱时才会公开（`ctx.shell.sandboxMode` 报告一个具有限制作用的默认值）：被拒命令所需的更宽模式，取自封闭的目标词汇 `workspace-write`/`danger-full-access`（绝不能缩减为执行器默认值；有效模式按会话确定。该模式已覆盖的已知目标会被忽略并按常驻策略运行，即使继承的 `justification` 为空或缺失；只有会改变模式的请求才需要在执行时做严格拓宽校验并获得批准）。 |
| `justification` | string | 必须与会改变模式的 `sandbox_permissions` 请求一同提供（缺少任一项都会产生验证错误）：用一句话向用户解释此命令为何需要这项更宽权限。常驻策略已覆盖的已知目标会被规范化处理，不会发起批准提示。 |
