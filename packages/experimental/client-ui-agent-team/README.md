...
Enable this package through [`@deepseek-ai/dsh-experimental-agent-team-profile`](../agent-team-profile/README.md), which supplies the Team service, tools, and Web UI together. The Web Client loader mounts the `/client` export; the root Host export is inert. Set `enabledPresets` to limit the header action to an explicit preset allowlist; when browser config is omitted, only preset ids beginning with `team-` render the action, and an explicit empty list shows it for every session.
