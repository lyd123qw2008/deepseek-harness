...
两个进程内驱动在创建子 agent 前都会通过 `resolveChildAgentOptions()` 快照父级已记录的活动提供方／模型路由和显式推理强度。它不会固定适配器实体化的推理强度：子 agent 会让自己的目标路由再次解析该默认值。若请求覆盖了提供方或模型，除非请求显式给出推理强度，否则它会从该路由的默认值开始；父级尚无请求 header 时则回退到其声明的选项。可继续创建会在提供方准备之前快照该结果，并把已解析的路由写入冷恢复用的记录；子 agent 已完成一次请求后，其自身记录的 header 会恢复显式推理强度。
