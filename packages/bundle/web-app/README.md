...
The Web profile also loads the dormant Codex subagent provider on the host; the shipped `codex` agent preset exposes `subagent_codex` and omits DSH's `web_search`, so network research uses Codex's native websearch.
