...
`AgentOptions` 提供初始的提供方／模型路由、可选的显式 `reasoningEffort` 以及可选的正数 `maxTokens` 输出上限。具体循环会把这些显式值用于全新请求；恢复时若存在匹配的已记录 header，则该 header 仍具有权威性，并会记录生效的推理强度和上限。省略的值由确切模型的适配器或提供方路由默认值处理。
