...
`AgentOptions` supplies the initial provider/model route, an optional explicit `reasoningEffort`, and an optional positive `maxTokens` output cap. The concrete loop uses those explicit values for a fresh request, while a matching logged header remains authoritative on resume; it records the effective effort and cap in that header. Omitted values let the exact-model adapter or provider route defaults apply.
