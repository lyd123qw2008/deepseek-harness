...
- **`SESSION_FORMAT_VERSION` stays pinned at `0`** — pre-release, no broad compatibility implied: `Session` accepts only current seed shapes, a backend refuses any other version, and an unknown event type refuses reconstruction unless its envelope carries `ignorable: true` ([event compatibility](../../../.agents/notes/implemented/architecture/2026-08-30-ignorable-session-events.md), [fail-closed rule](../../../.agents/notes/implemented/simplification/2026-08-25-fail-closed-session-event-vocabulary.md)).
