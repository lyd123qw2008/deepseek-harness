...
- **`SESSION_FORMAT_VERSION` 固定为 `0`**：预发布阶段不承诺广泛兼容性；`Session` 只接受当前 seed 形状，后端拒绝任何其他版本；不认识的事件类型只有在其 envelope 带有 `ignorable: true` 时才可跳过（[事件兼容性](../../../.agents/notes/implemented/architecture/2026-08-30-ignorable-session-events.zh.md)，[fail-closed 规则](../../../.agents/notes/implemented/simplification/2026-08-25-fail-closed-session-event-vocabulary.zh.md)）。
