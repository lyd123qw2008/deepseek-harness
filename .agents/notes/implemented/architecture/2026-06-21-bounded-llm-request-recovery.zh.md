...
- 各提供方适配器公开的事实并不相同。经过补丁处理的 pi-ai OpenAI Responses 与 Codex 路径会在可用时保留提供方 code、有效 HTTP 状态、Retry-After 和请求 id；其他 API 可能将失败压平成文本，因此适配器先使用结构化事实，只有没有结构化元数据时才进行窄范围的消息分类。
- 重试归属因适配器而异。手写 DeepSeek 适配器只尝试一次；pi-ai 适配器禁用库内部重试，并将 Codex 固定为 SSE，因此一次流调用不会隐藏 auto transport 的重放。把隐藏的传输重试与 `agent/request-error` 监听器结合，会成倍增加尝试次数，并使中间失败无法记入会话日志。
- 恢复后的失败携带持久状态事实。失败的步骤和分片仍可重建，观察者也能得知 agent（智能体）是否在退避、将等待多久以及等待原因。
...
适配器每次调用 `stream()` 只执行一次提供方模型生成请求。pi-ai 适配器移除公开的 `maxRetries` 和 `maxRetryDelayMs` profile 字段，并禁用库内部重试；Codex 路由还会固定使用 `transport: 'sse'`，避免 auto transport 的 WebSocket `response.create` 请求再通过 SSE 重放。手写适配器保持现有的单次尝试行为。这样既避免 SDK 预算成倍放大 agent 预算，又能确保每次暂时性重试都由一个已关闭的失败步骤加 `llm/retry` 表示。
...
- pi-ai 将 SDK 选项固定为零次重试，并针对可重试的提供方响应执行一次可观测的线路请求尝试；Codex 路由使用 SSE，避免 auto transport 重放。独立测试确保移除任一边界都会失败。
