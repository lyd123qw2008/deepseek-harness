...
`terminalCardModel` applies the same eligibility checks to root and PTC dispatch calls, without rejecting `parentCallId`. Supported running and settled `bash`, `pwsh`, and `terminal_send` calls use the existing terminal card. Background calls, tool errors, malformed inputs, missing call heads, and unsupported result content retain generic fallback. A malformed optional escalation pair retains that fallback only while the call has no successful result: a settled successful call keeps its card ([Terminal card survives an invalid escalation pair](2026-09-21-terminal-card-invalid-escalation-pair.md)). Persistent shells remain eligible while running and generic when settled; a nonzero process exit remains terminal result data rather than a tool error.
