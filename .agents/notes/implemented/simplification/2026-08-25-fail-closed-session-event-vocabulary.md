...
Every session event type is required-on-read unless the record carries the explicit migration compatibility marker described in the [marked-event compatibility note](../architecture/2026-08-30-ignorable-session-events.md). After supported legacy records are normalized, `PersistenceCoordinator` compares each event type with `KNOWN_SESSION_EVENT_TYPES`, the generated set of every `SessionEventMap` member declared in this repository. Any unknown unmarked type refuses reconstruction with `SessionFormatUnsupportedError`; the diagnostic names the event and sequence, identifies the likely newer writer, and includes the raw artifact path when the backend has one. The guard remains read-side only because rejecting an append after a live event is committed would interrupt durability before the session can report the unsupported log on its next load.

`SessionEvent.ignorable` is an optional envelope field accepted for explicitly informational migrated records. JSONL preserves the field and `SESSION_FORMAT_VERSION` remains `0`. The SQLite provider replaces the overloaded `ignorable` column with the schema-18 `is_packed` discriminator: scalar logical events store `0`, packed chunk rows store `1`, and an event name equal to a physical chunk tag remains unambiguous before the coordinator applies the known-type guard. The current Alpha migration uses JSONL; the SQLite physical format does not provide this marker path and therefore remains fail-closed for an unrecognized event.
...
An older build cannot resume a newer same-version log once that log contains any unmarked event type it does not know, even when the new event is informational. The Alpha migration restores the explicit marker only for existing JSONL records whose producer established that they do not affect reconstruction; it does not provide a general external-event registration mechanism. A future producer that needs this behavior beyond the migration must classify the event type once, make `Session.append()` emit the persisted classification automatically, and cover every persistence backend and the wire representation.
