...
The component shares the TUI's single-column framing, line-terminator rule, and distinct-path file count. The Web renderer reconstructs semantic context, removed, and added rows from each stored hunk, so context is neutral instead of being repeated as both a deletion and an addition. Applied hunks carry optional 1-based `oldStart`/`newStart` values for file line gutters; call-time and older session hunks without those fields still render with the marker columns only.
...
**Git-style line-number gutters.** Accepted: `FileDiff` now carries optional 1-based `oldStart`/`newStart` hunk starts, populated from `structuredPatch` by `tool-fs`. The fields are optional so call-time presenters and older session metadata remain readable; the Web primitive falls back to marker columns when they are absent. The TUI can adopt the same fields without changing the shared wire vocabulary.
