...
`dsh-tool-fs` 把当前会话解析成完整策略，并传给每次变更，同时将 `FS_SANDBOX_DENIED` 映射为模型已从 bash 认识的标记：`[sandbox: file access denied under <mode> mode]`。当 `ctx.fs.sandboxMode` 在注册时报告一个受限模式，`write` 与 `edit` 宣告相同的 `sandbox_permissions` + `justification` 字段，向模型说明同样的同一轮次重试方式，并在执行前处理同样的 `ctx.approval` 请求。有效模式已经覆盖的已知目标会在工具消费方被规范化处理；每个会改变权限的请求仍保留[沙箱 Agent Note](2026-07-06-sandbox.md) § 升级中的共享严格放宽检查、批准结果和逐字 fail-closed 文案。授权只改变当前调用的模式，并保留其会话根目录；不产生任何新会话事件（见[已覆盖字段修复](../bug-fix/2026-08-14-redundant-sandbox-escalation-fields.md)）。
