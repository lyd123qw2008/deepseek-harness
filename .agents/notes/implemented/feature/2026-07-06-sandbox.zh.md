...
- **默认值相对的升级阶梯（仅公布比执行器注册时默认值更宽的模式）**：否决。按会话覆盖使默认值成为错误的基线——切换到比默认值更窄的会话恰恰失去它需要的杠杆，而在 `danger-full-access` 默认值下字段完全消失，同时一个被覆盖为 `read-only` 的会话仍处于约束中却没有升级路径。枚举固定封闭的目标词汇；会改变权限的请求会按调用基于会话有效模式做严格放宽检查，而已被覆盖的已知目标会在工具消费方被规范化处理并按常驻策略运行（见[已覆盖字段修复](../bug-fix/2026-08-14-redundant-sandbox-escalation-fields.zh.md)）。
- **按会话动态工具 schema**：否决。schema 设计上是注册表全局的（一套 assembly 词汇、固定 header 快照约定），按会话重新注册只能买到按调用规范化与会改变权限的严格放宽检查已经保证的东西，代价是按会话的 schema 和每次切换的 header 变动。
...
- 升级字段恰好在已挂载的执行器约束时存在；调用有效模式已经覆盖的已知目标会被规范化处理并按该常驻策略运行，而每个会改变权限的请求都必须严格更宽、携带有效的成对参数并获得批准。没有 ApprovalService 的部署会让会改变权限的升级失败关闭，而普通和冗余调用不受影响。
...
- **公布的目标集是静态的，而有效模式是按会话的**（schema 是注册表全局的）——已处于最宽模式的会话仍被提供这些字段。构造上无害：不增加权限的已知目标会被规范化处理并按常驻策略运行；任何会改变权限的请求仍受严格放宽检查和批准门控（见[已覆盖字段修复](../bug-fix/2026-08-14-redundant-sandbox-escalation-fields.zh.md)）。
