...
**web 客户端的默认文件动作由[恢复 Host 打开行为的修正](../bug-fix/2026-09-09-restore-host-file-opening.zh.md)恢复。** Chat 与 Tool 的路径链接再次相对于 Session 工作区解析并调用 `session/openWorkspacePath`；右侧 Sidebar 仍作为独立的文件树与文本预览界面存在。Host 打开器仍负责桌面交接；下面的七月决定原文记录了它为何是本机 Web 部署的安全默认项。
...
现有的每一处文件交互仍然汇到 `openFile`，因此 write、edit、read 与通用单文件行共享一条 Host 打开路径，不需要逐行分支。右侧 Sidebar 仍是独立的工作区浏览与文本预览界面，不会替代 Chat 与 Tool 文件链接的默认动作。产出文件 chip 保留 `title` 中的完整路径；通过 Host 打开时，Markdown 仍交给平台的 `.md` 应用。
