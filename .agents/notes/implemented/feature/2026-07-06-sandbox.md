...
- **A default-relative escalation ladder (advertise only the modes wider than the executor's registration-time default)** — rejected: per-session overrides make the default the wrong baseline — a session switched narrower than the default loses exactly the lever it needs, and under a `danger-full-access` default the fields vanish entirely while a `read-only`-overridden session stays confined with no escalation path. The enum pins the closed target vocabulary; an authority-changing request is a per-call strict-widening check against the session's effective mode, while an already-covered known target is normalized at the tool consumer and runs under its standing policy ([the covered-fields fix](../bug-fix/2026-08-14-redundant-sandbox-escalation-fields.md)).
- **Per-session dynamic tool schemas** — rejected: schemas are registry-global by design (one assembly vocabulary, the pinned-header snapshot contract), and re-registering per session would buy only what per-call normalization plus the authority-changing strict-wider check already guarantee, at the cost of a per-session schema and header churn on every switch.
...
- The escalation fields exist exactly when the mounted executor confines; a known target already covered by the call's effective mode is normalized and runs under that standing policy, while every authority-changing request must be strictly wider, carry valid paired arguments, and obtain approval. A deployment with no ApprovalService fails authority-changing escalations closed and leaves plain and redundant calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: a known target that adds no authority is normalized and runs under the standing policy; any request that changes authority remains subject to the strict-wider check and approval gate ([the covered-fields fix](../bug-fix/2026-08-14-redundant-sandbox-escalation-fields.md)).
